...
contract AddressBookMock {
    ...
    function mockRegisterCnStakingContracts(
        address[] memory _cnNodeIdList,
        address[] memory _cnStakingContractAddressList,
        address[] memory _cnRewardAddressList
    ) external {
        require(
    ...
    function mockUnregisterCnStakingContracts(address[] memory _cnNodeIdList) external {
        for (uint i = 0; i < _cnNodeIdList.length; i++) {
            // skip duplicate
            address _cnNodeId = _cnNodeIdList[i];
            uint256 index = cnIndexMap[_cnNodeId];
